...
const files = git([ 'ls-files' ], { env }).trim().split(/\n/)
const length = files.length
let i = 0
const authors = { }

for(const file of files) {
	const lines = git([ 'blame', '--line-porcelain', 'HEAD', file, ...args ], { env }).trim().split(/\n/)
...
const formatNumber = num => {
	...
	const { length } = str
	return [ ...Array(length) ].map((_, i) => str[length - 1 - i] + (i > 0 && i % 3 === 0 ? ',' : '')).reverse().join('')
}
